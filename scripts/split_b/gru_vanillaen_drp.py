...
def build_model(flag='training'):
    ...
    if flag == 'testing':
        print('--- Loading Model ---')
        pcen.load_state_dict(torch.load('results/split_b/vanillaen_pcen_bs_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
        gru_enc.load_state_dict(torch.load('results/split_b/vanillaen_gru_enc_bs_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
        gru_dec.load_state_dict(torch.load('results/split_b/vanillaen_gru_dec_bs_drp.pytorch',  map_location={'cuda:1': 'cpu:0'}))
        fc_layer.load_state_dict(torch.load('results/split_b/vanillaen_cls_bs_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
    if flag == 'training':
...
def perform_training():
    ...
    training_keys = sorted(list(data_dict.keys()))[0:1]  # Train on the first two composers
        ...
        if not epoch == 1:
            ...
            if prv_cls_error - cls_err > 0:
                ...
                if cls_err < best_error:
                    ...
                    best_error = cls_err
                    print('--- Saving Model ---')
                    torch.save(pcen.state_dict(), 'results/split_b/vanillaen_pcen_bs_drp.pytorch')
                    torch.save(gru_enc.state_dict(), 'results/split_b/vanillaen_gru_enc_bs_drp.pytorch')
                    torch.save(gru_dec.state_dict(), 'results/split_b/vanillaen_gru_dec_bs_drp.pytorch')
                    torch.save(fc_layer.state_dict(), 'results/split_b/vanillaen_cls_bs_drp.pytorch')
...
def perform_validation(nn_list):
    ...
    validation_key = keys[1]  # Validate on the second composer
