...
def build_model(flag='training'):
    ...
    if flag == 'testing':
        print('--- Loading Model ---')
        cnn_block.load_state_dict(torch.load('results/split_b/cnn_zmean_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
        gru_enc.load_state_dict(torch.load('results/split_b/cnn_gru_enc_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
        gru_dec.load_state_dict(torch.load('results/split_b/cnn_gru_dec_drp.pytorch',  map_location={'cuda:1': 'cpu:0'}))
        fc_layer.load_state_dict(torch.load('results/split_b/cnn_cls_drp.pytorch', map_location={'cuda:1': 'cpu:0'}))
    if flag == 'training':
        ...
        if not epoch == 1:
            ...
            if prv_cls_error - cls_err > 0:
                ...
                if cls_err < best_error:
                    ...
                    best_error = cls_err
                    print('--- Saving Model ---')
                    torch.save(cnn_block.state_dict(), 'results/split_b/cnn_zmean_drp.pytorch')
                    torch.save(gru_enc.state_dict(), 'results/split_b/cnn_gru_enc_drp.pytorch')
                    torch.save(gru_dec.state_dict(), 'results/split_b/cnn_gru_dec_drp.pytorch')
                    torch.save(fc_layer.state_dict(), 'results/split_b/cnn_cls_drp.pytorch')
            else:
